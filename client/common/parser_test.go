...
func TestReadBatchHandlesLeftoverLine(t *testing.T) {
	...
	maxBatch := 2
	maxSize := len(header) + len(aStringBet) + 1 // Tamaño que representa la primera línea + 1 byte
	...
	parser := &Parser{
		file:         file,
		maxBatch:     maxBatch, // La idea es que se procese un batch de 2 líneas por llamda
